    ...
    <div className="flex flex-col h-full bg-[#FAFAFA] p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <div className="flex gap-3 w-full sm:w-auto">
          <button
            ...
            className="btn-secondary h-10 px-4 text-[13px] font-semibold bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 rounded-[20px] flex items-center justify-center gap-2"
          >
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6 justify-between items-center">
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            ...
            className="w-full text-[14px] font-medium rounded-[20px] pl-9 pr-4 py-2.5 bg-white border border-slate-200 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-100 transition-all "
          />
        </div>
      </div>

      {/* Data Table */}
      <div className="bg-white rounded-[20px] border border-slate-100  flex-1 overflow-hidden flex flex-col">
              ...
              {filteredPatients.length === 0 ? (
                ...
                filteredPatients.map((patient) => (
                  <tr
                    ...
                    className="hover:bg-slate-50/50 hover: transition-all cursor-pointer group"
                  >
                    ...
                    <td className="px-6 py-3 whitespace-nowrap">
                      <span
                        className={`rounded-[30px] px-2.5 py-1 text-[11px] font-semibold border ${
                          patient.insuranceType === 'CNOPS'
                            ? 'bg-purple-50 text-purple-700 border-purple-200/50 mix-blend-multiply'
                    ...
                    <td className="px-6 py-3 whitespace-nowrap text-right text-sm font-medium">
                      <button
                        ...
                        className="text-slate-400 hover:text-slate-900 transition-colors p-2 rounded-[20px] hover:bg-slate-100"
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-white flex justify-between items-center text-[12px] text-slate-500 font-semibold tracking-tight">
          <span>Affichage de 1 à {filteredPatients.length} patients</span>
          <div className="flex gap-2">
            <button
              disabled
              className="px-3 py-1.5 bg-white border border-slate-200 rounded-[30px] hover:bg-slate-50 disabled:opacity-50 text-[13px]"
            >
              Précédent
            </button>
            <button className="px-3 py-1.5 bg-white border border-slate-200 rounded-[30px] hover:bg-slate-50 text-[13px]">
              Suivant
            </button>
          </div>
        </div>
          ...
          <div className="flex flex-col h-full bg-slate-50/50">
            ...
            <div className="p-8 space-y-8 flex-1 overflow-y-auto scrollbar-hide">
                    ...
                    {!isEditing ? (
                      <>
                        <div className="flex items-center gap-6 mb-8">
                          <div className="w-20 h-20 bg-slate-50 border border-slate-200 rounded-[20px] flex items-center justify-center text-3xl font-bold text-slate-400">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-2">
                              <span className="rounded-[30px] px-2 py-0.5 bg-slate-100 text-slate-700 text-[11px] font-bold">
                                {selectedPatient.age || 0} ans
                              </span>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <input
                                ...
                                className="w-full h-9 rounded-[20px] border border-slate-200 bg-white px-3 text-[13px] font-medium focus:ring-2 focus:ring-slate-100 focus:border-slate-500 transition-all outline-none"
                              />
                            </div>
                            <div>
                              <label className="text-[13px] font-semibold text-slate-700 mb-1 block">
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className="bg-red-50 border border-red-200 rounded-[20px] p-5 ">
                    ...
                    <p className="text-[13px] text-slate-600 italic">
                      Gérez les allergies depuis le dossier Patient détaillé.
                    </p>
                  </div>
                  <div className="bg-orange-50 border border-orange-200 rounded-[20px] p-5 ">
                    ...
                    <p className="text-[13px] text-slate-600 italic">
                      Gérez les antécédents depuis le dossier Patient détaillé.
                    </p>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'history' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  {getPatientAppointments(selectedPatient.id).length > 0 ? (
                      ...
                      {getPatientAppointments(selectedPatient.id).map((apt) => (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-[20px] flex items-center justify-center shrink-0 border border-blue-100">
                          ...
                          <div
                            className={cn(
                              'text-[11px] font-bold px-2.5 py-1 rounded-[30px] border tracking-wide uppercase',
                              apt.status === 'Terminé'
                    ...
                    <div className="py-20 flex flex-col items-center justify-center bg-white rounded-[20px] border-2 border-dashed border-slate-100 opacity-60">
              ...
              {['finance', 'docs'].includes(activeTab) && (
                <div className="py-20 flex flex-col items-center justify-center bg-white rounded-[20px] border-2 border-dashed border-slate-100 opacity-60 animate-in fade-in duration-300">
                  {activeTab === 'finance' ? (
                    <IconCreditCard className="w-10 h-10 text-slate-300 mb-4" />
                  ) : (
                    <IconFolder className="w-10 h-10 text-slate-300 mb-4" />
                  )}
                  <h5 className="text-[16px] font-bold text-slate-900 tracking-tight">
                    Prochainement
                  </h5>
      ...
      >
        <form
          ...
          className="p-8 space-y-8 bg-slate-50/50 h-full overflow-y-auto scrollbar-hide"
        >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full h-9 rounded-[20px] border border-slate-200 bg-white px-3 text-[13px] focus:ring-2 focus:ring-slate-100 focus:border-slate-500 outline-none"
                />
              </div>
              <div>
          ...
          <div className="flex gap-4">
            <button
              ...
              className="h-10 px-4 text-[13px] font-bold text-slate-700 border border-slate-200 bg-white hover:bg-slate-50 rounded-[30px] flex-1"
            >
              Annuler
            </button>
            <button
              type="submit"
              className="h-10 px-4 text-[13px] font-bold text-white bg-[#0F0F0F] hover:bg-black rounded-[30px] flex-1"
            >
              Créer Dossier
            </button>
          </div>
      ...
      {isImportOpen && (
        <SlideOver
          ...
          title="Import CSV"
          subtitle="Importer des patients en masse"
        >
          <div className="p-8">
            <p className="text-[13px] text-slate-600">
              L'importation de fichiers CSV sera disponible prochainement.
            </p>
          </div>
        </SlideOver>
      )}
