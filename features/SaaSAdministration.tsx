  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex bg-slate-100/80 p-1 rounded-[20px] border border-slate-200/60 ">
          <button
            onClick={() => setActiveTab('templates')}
            className={`px-4 py-1.5 text-[13px] font-medium rounded-[6px] transition-all ${activeTab === 'templates' ? 'bg-white text-slate-900 ' : 'text-slate-500 hover:text-slate-900'}`}
          >
            Templates (Email/SMS)
          </button>
          <button
            onClick={() => setActiveTab('pages')}
            className={`px-4 py-1.5 text-[13px] font-medium rounded-[6px] transition-all ${activeTab === 'pages' ? 'bg-white text-slate-900 ' : 'text-slate-500 hover:text-slate-900'}`}
          >
            Pages Légales
          </button>
          <button
            onClick={() => setActiveTab('translations')}
            className={`px-4 py-1.5 text-[13px] font-medium rounded-[6px] transition-all ${activeTab === 'translations' ? 'bg-white text-slate-900 ' : 'text-slate-500 hover:text-slate-900'}`}
          >
            Traductions
          </button>
        </div>
      ...
      {activeTab === 'templates' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {templates.map((tmpl) => (
            <div
              key={tmpl.id}
              className="bg-white border border-slate-200/60 rounded-[20px] p-5 hover: transition-shadow group"
            >
              <div className="flex justify-between items-start mb-4">
                <div
                  className={`p-2.5 rounded-[20px] border ${tmpl.type === 'Email' ? 'bg-blue-50/50 text-blue-600 border-blue-100/50' : 'bg-emerald-50/50 text-emerald-600 border-emerald-100/50'}`}
                >
                  {tmpl.type === 'Email' ? (
              ...
              <p className="text-[12px] text-slate-500 mb-4 truncate font-medium">
                Sujet: {tmpl.subject}
              </p>
          ...
          <button className="border-2 border-dashed border-slate-200/80 rounded-[20px] p-6 flex flex-col items-center justify-center text-slate-400 hover:border-blue-400/50 hover:bg-slate-50 hover:text-blue-600 transition-all cursor-pointer">
      ...
      {/* PAGES TAB */}
      {activeTab === 'pages' && (
        <div className="bg-white border border-slate-200/60 rounded-[20px] overflow-hidden">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {translations.map((mod) => (
              <div key={mod.id} className="bg-white border border-slate-200/60 rounded-[20px] p-5 ">
                ...
                <div className="w-full bg-slate-100 border border-slate-200/50 rounded-full h-1.5 mb-5 ">
                  <div
                ...
                <div className="flex justify-between items-center pt-4 border-t border-slate-100/80 text-[12px] font-medium">
                  ...
                  <button className="text-blue-600 font-bold hover:text-blue-700 hover:underline transition-all">
                    Gérer
                  </button>
                </div>
              </div>
        ...
        {editingTemplate && (
          <div className="p-6 space-y-6 flex flex-col h-full bg-[#FAFAFA]">
            <div>
              ...
              <input
                type="text"
                className="w-full border border-slate-200/60 rounded-[20px] p-2.5 text-[14px] bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
            ...
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <textarea
                className="w-full flex-1 border border-slate-200/60 rounded-[20px] p-3 text-[13px] font-mono focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none bg-slate-50 resize-none transition-all leading-relaxed"
                value={templateBody}
            ...
            <div className="pt-4 flex gap-3 border-t border-slate-200/60 mt-auto">
              <button
                ...
                className="sa-btn-ghost flex-1 cursor-pointer"
              >
                Annuler
              </button>
              <button onClick={handleSaveTemplate} className="sa-btn flex-1 cursor-pointer">
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-0.5">
            Revenus, plans d'abonnement et métriques financières.
          </p>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[20px] border border-slate-200/60 flex flex-col justify-between h-32">
      ...
      <div className="card p-6">
        <h3 className="text-[14px] font-semibold text-slate-900 mb-5 tracking-tight">
          Plans d'abonnement
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {MOCK_PLANS.map((plan) => (
            <div
              key={plan.id}
              className="border border-slate-200/60 rounded-[20px] p-5 hover:border-[#136cfb]/30 hover: transition-all relative group bg-slate-50/30"
            >
              {plan.isPopular && (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {addons.map((addon) => (
          <div
            key={addon.id}
            className="bg-white border border-slate-200/60 rounded-[20px] p-5 hover: transition-shadow"
          >
            <div className="flex justify-between items-start mb-4">
              <div className="p-2.5 bg-blue-50/50 text-[#136cfb] rounded-[20px] border border-blue-100/50">
                {addon.icon === 'message' ? (
                  <IconMail className="w-5 h-5" />
                ) : addon.icon === 'wand' ? (
            ...
            <p className="text-[13px] text-slate-500 mt-1.5 h-10 leading-snug">
              {addon.description}
            </p>
            ...
            <div className="mt-5 pt-4 border-t border-slate-100 flex justify-between items-center">
              <div className="font-bold text-slate-900">
                {addon.price} MAD{' '}
                <span className="font-medium text-[11px] text-slate-400">/mois</span>
              </div>
              <div className="text-[12px] font-bold text-slate-500">
                {addon.activeInstalls} installs
              </div>
            </div>
    ...
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-0.5">
            Envoyez des messages et annonces à vos utilisateurs.
          </p>
        </div>
        <button
            ...
            >
              <div className="flex items-center justify-between mb-4 px-1">
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${status === 'Sent' ? 'bg-emerald-500 ' : status === 'Scheduled' ? 'bg-blue-500 ' : 'bg-slate-300'}`}
                  ></div>
                ...
                {broadcasts
                  ...
                  .map((b) => (
                    <div
                      ...
                      className="bg-white p-4 rounded-[20px] border border-slate-200/60 cursor-grab active:cursor-grabbing hover: transition-shadow"
                    >
      ...
      {isComposeOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          ></div>
          <div className="bg-white rounded-[20px] w-full max-w-md relative z-10 p-6 space-y-5 border border-slate-200/60">
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-0.5">Gestion des modèles LLM et coûts opérationnels.</p>
        </div>
        <div className="text-right">
          <div className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Coût IA / jour</div>
          <span className="text-[22px] font-semibold text-slate-900 tracking-tight">
            185.40 <span className="text-[13px] font-medium text-slate-400">USD</span>
          </span>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-[20px] border border-slate-200/60 ">
        ...
        <div className="space-y-4">
          {MOCK_AI_CONFIGS.map((config) => (
            <div
              key={config.id}
              className="bg-white p-5 rounded-[20px] border border-slate-200/60 "
            >
              <div className="font-bold text-[14px] text-slate-900">{config.name}</div>
              <div className="text-[12px] text-slate-500 mt-1 font-medium">
                {config.provider} •{' '}
                <span className="text-indigo-600 font-bold">${config.costPer1kTokens}</span>/1k tk
              </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-slate-500 mt-0.5">
            Suivez le risque d'attrition et la santé du parc clients.
          </p>
        </div>
      </div>
      <div className="bg-emerald-50 rounded-[20px] p-6 border border-emerald-100 flex items-center justify-between">
        <div>
          <h3 className="text-[14px] font-bold text-emerald-900 mb-1">Santé du Parc Clients</h3>
          <p className="text-[12px] text-emerald-700/80 mt-1 font-medium">
            Vue globale sur le risque d'attrition
          </p>
        </div>
          ...
          <tbody className="divide-y divide-slate-100/80">
            {risks.map((risk) => (
              <tr key={risk.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-6 py-4 text-right">
                  <button className="text-[12px] text-blue-600 font-bold hover:text-blue-700 transition-colors">
                    Contacter
                  </button>
                </td>
              </tr>
...
export const BackupManagerPage = () => (
  <div className="space-y-6">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 className="text-[22px] font-semibold text-slate-900 tracking-tight">Data & Backups</h1>
        <p className="text-[13px] text-slate-500 mt-0.5">
          Gérez les sauvegardes et la rétention des données.
        </p>
      </div>
    </div>
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-white p-6 rounded-[20px] border border-slate-200/60 ">
          ...
          {MOCK_BACKUPS.map((bk) => (
            <div key={bk.id} className="py-3 flex justify-between items-center">
              <div className="text-[13px] text-slate-800 font-medium">
                {bk.clinicName}{' '}
                <span className="text-slate-400 font-normal ml-1">• {bk.createdAt}</span>
              </div>
              <span className="text-[11px] font-bold bg-emerald-50 text-emerald-700 px-2 py-1 rounded-[4px] border border-emerald-100/50">
                Réussi
              </span>
            </div>
          ))}
  ...
  <div className="space-y-6">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        ...
        <p className="text-[13px] text-slate-500 mt-0.5">
          Supervisez l'état des serveurs et les performances de l'infrastructure.
        </p>
      </div>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {MOCK_REGIONS.map((region) => (
        <div
          key={region.id}
          className="bg-[#0F0F0F] rounded-[20px] p-6 text-white border border-slate-800 relative overflow-hidden group"
        >
          ...
          <div className="relative z-10">
            ...
            <div className="text-2xl font-bold mt-1 text-slate-50 tracking-tight">
              {region.name}
            </div>
            <div className="text-[13px] text-slate-400 mt-3 flex items-center gap-1.5">
              <IconGlobe className="w-3.5 h-3.5" /> {region.activeTenants} Tenants
            </div>
          </div>
        </div>
      ))}
    ...
    <div className="bg-white p-6 rounded-[20px] border border-slate-200/60 ">
      <h4 className="text-[13px] font-bold text-slate-900 mb-6 flex items-center gap-2">
        <IconActivity className="w-4 h-4 text-emerald-500" /> Cache Hit Rates
      </h4>
...
export const CompliancePage = () => (
  <div className="space-y-6">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 className="text-[22px] font-semibold text-slate-900 tracking-tight">Compliance Center</h1>
        <p className="text-[13px] text-slate-500 mt-0.5">
          Gardez un œil sur la conformité légale et la souveraineté des données.
        </p>
      </div>
    </div>
    <div className="bg-white border border-slate-200/60 rounded-[20px] overflow-hidden">
...
export const RoadmapPage = () => (
  <div className="space-y-6">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 className="text-[22px] font-semibold text-slate-900 tracking-tight">Product Roadmap</h1>
        <p className="text-[13px] text-slate-500 mt-0.5">
          Visualisez les requêtes fonctionnalités et définissez les priorités.
        </p>
      </div>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {['Planned', 'In Progress', 'Live'].map((status) => (
        <div key={status} className="space-y-4">
          <div className="text-[13px] font-bold text-slate-800 border-b border-slate-200/60 pb-3">
            {status}
          </div>
          {MOCK_FEATURE_REQUESTS.filter((f) => (status === 'Live' ? f.status === 'Live' : true))
            ...
            .map((f) => (
              <div
                key={f.id}
                className="bg-white p-4 rounded-[20px] border border-slate-200/60 hover: transition-shadow cursor-default group"
              >
                <div className="font-bold text-[13px] text-slate-900 group-hover:text-blue-600 transition-colors">
                  {f.title}
                </div>
...
export const SecurityPage = () => (
  <div className="space-y-6">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 className="text-[22px] font-semibold text-slate-900 tracking-tight">Sécurité</h1>
        <p className="text-[13px] text-slate-500 mt-0.5">
          Journal des événements de sécurité et alertes.
        </p>
      </div>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
      <div className="bg-[#0F0F0F] p-5 rounded-[20px] text-white border border-gray-800 flex flex-col justify-between h-32 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="text-[32px] font-semibold text-emerald-400 tracking-tight mt-1 flex items-center gap-2">
            Low <IconCheckCircle className="w-6 h-6" />
          </div>
        </div>
      </div>
    </div>
        ...
        <tbody className="divide-y divide-slate-200/60">
          {MOCK_SECURITY_EVENTS.map((s) => (
            <tr key={s.id} className="hover:bg-slate-50/50 transition-colors">
              ...
              <td className="px-6 py-4 text-right">
                <span
                  className={`px-2 py-1 rounded-[4px] text-[11px] font-bold border ${s.threatLevel === 'High' ? 'bg-red-50 text-red-700 border-red-100/50' : s.threatLevel === 'Medium' ? 'bg-orange-50 text-orange-700 border-orange-100/50' : 'bg-emerald-50 text-emerald-700 border-emerald-100/50'}`}
                >
                  {s.threatLevel}
                </span>
              </td>
...
export const SystemHealthPage = () => (
  <div className="space-y-6">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 className="text-[22px] font-semibold text-slate-900 tracking-tight">System Health</h1>
        <p className="text-[13px] text-slate-500 mt-0.5">
          Métriques de performance de la plateforme en temps réel.
        </p>
      </div>
    </div>
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 bg-white p-6 border border-slate-200/60 rounded-[20px]">
      ...
      <div className="bg-white p-6 border border-slate-200/60 rounded-[20px] flex flex-col">
        <h4 className="text-[13px] font-bold text-slate-900 mb-6 flex items-center justify-between">
          Active Jobs
          <span className="bg-blue-50 text-blue-600 font-bold px-2 py-0.5 rounded-[4px] text-[10px]">
            {MOCK_JOBS.length} Queue
          </span>
        </h4>
...
export const DevelopersPage = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-semibold text-slate-900 tracking-tight">Développeurs</h1>
          <p className="text-[13px] text-slate-500 mt-0.5">Intégrations, API, et paramètres avancés.</p>
        </div>
      </div>
              ...
              >
                <div>
                  ...
                  <div className="text-[12px] font-medium text-slate-400 mt-1.5 flex items-center gap-1.5">
                    <span className="text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded-[4px] border border-slate-200/50">
                      Events
                    </span>{' '}
                    {w.events.join(', ')}
                  </div>
                </div>
                <div className="text-[11px] font-bold bg-emerald-50 text-emerald-700 px-2 py-1 rounded-[4px] border border-emerald-100/50">
                  Active
                </div>
              </div>
            ))}
      ...
      {subTab === 'api' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {MOCK_API_METRICS.map((m) => (
            <div
              key={m.endpoint}
              className="p-6 border border-slate-200/60 rounded-[20px] bg-white relative overflow-hidden group hover: transition-shadow"
            >
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <IconCode className="w-10 h-10 text-slate-400" />
              </div>
              <div className="text-[12px] font-mono font-bold text-slate-500 truncate relative z-10">
                {m.endpoint}
              </div>
              <div className="text-[28px] font-bold text-slate-900 mt-3 relative z-10 tracking-tight">
                {m.requests.toLocaleString()}{' '}
                <span className="text-[14px] font-medium tracking-normal text-slate-400">reqs</span>
              </div>
              <div className="text-[12px] font-bold mt-5 relative z-10 flex gap-4">
                <span className="text-slate-500 bg-slate-50 px-2.5 py-1 rounded-[4px] border border-slate-200/50">
                  Latency: {m.avgLatency}ms
                </span>
                <span
                  className={`px-2.5 py-1 rounded-[4px] border ${m.errors > 0 ? 'bg-red-50 text-red-600 border-red-100/50' : 'bg-emerald-50 text-emerald-600 border-emerald-100/50'}`}
                >
                  Err: {m.errors}
                </span>
              </div>
      ...
      {subTab !== 'api' && subTab !== 'webhooks' && (
        <div className="p-10 text-center bg-slate-50/50 border border-slate-200/60 border-dashed rounded-[20px]">
...
export const SaaSAdministration = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      default:
        return (
          <div className="text-slate-400 font-medium text-center py-10 tracking-tight">
  ...
  return (
    <div className="flex relative h-[min(calc(100vh-140px),900px)] min-h-[600px] bg-white rounded-[20px] border border-slate-200/60 overflow-hidden font-sans">
      ...
      <div className="flex-1 overflow-auto bg-white p-6 lg:p-8 custom-scrollbar relative z-0">
        <div className="max-w-full w-full mx-auto h-full flex flex-col pb-10">
          {renderContent()}
        </div>
      </div>
    </div>
