...
import { 
  ...
  MOCK_TEMPLATES, MOCK_CONTENT, MOCK_TRANSLATIONS
} from '../constants';
import { 
  ...
  IconServer, IconMap, IconScale, IconCheck, IconGlobe, IconWand, IconShoppingBag, IconTrendingUp, IconHardDrive, IconDownload, IconPlay, IconMoreHorizontal, IconX, IconSend, IconFileText, IconEye, IconMessage
...
export const ContentManagementPage = () => {
    ...
    const handleEditTemplate = (tmpl: EmailTemplate) => {
        ...
        setTemplateBody(`Bonjour {userName},\n\nCeci est le contenu du modèle "${tmpl.name}".\n\nCordialement,\nL'équipe Medicom.`);
    };

    const handleSaveTemplate = () => {
        if (editingTemplate) {
            setTemplates(templates.map(t => t.id === editingTemplate.id ? { ...t, lastModified: 'Just now' } : t));
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h3 className="text-lg font-bold text-slate-900">Content Management System</h3>
                    <p className="text-sm text-slate-500">Gérez les communications, pages légales et traductions.</p>
                </div>
                <div className="flex bg-slate-100 p-1 rounded-md">
                    <button 
                        onClick={() => setActiveTab('templates')}
                        className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${activeTab === 'templates' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Templates (Email/SMS)
                    </button>
                    <button 
                        onClick={() => setActiveTab('pages')}
                        className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${activeTab === 'pages' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Pages Légales
                    </button>
                    <button 
                        onClick={() => setActiveTab('translations')}
                        className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${activeTab === 'translations' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Traductions
                    </button>
                </div>
            ...
            {activeTab === 'templates' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {templates.map(tmpl => (
                        <div key={tmpl.id} className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm hover:shadow-md transition-all group">
                            <div className="flex justify-between items-start mb-4">
                                <div className={`p-2 rounded-lg ${tmpl.type === 'Email' ? 'bg-blue-50 text-blue-600' : 'bg-green-50 text-green-600'}`}>
                                    {tmpl.type === 'Email' ? <IconMail className="w-5 h-5" /> : <IconMessage className="w-5 h-5" />}
                                </div>
                                <button onClick={() => handleEditTemplate(tmpl)} className="text-slate-400 hover:text-indigo-600">
                                    <IconEdit className="w-4 h-4" />
                                </button>
                            </div>
                            <h4 className="font-bold text-slate-900 text-sm mb-1">{tmpl.name}</h4>
                            <p className="text-xs text-slate-500 mb-4 truncate">Sujet: {tmpl.subject}</p>
                            <div className="pt-3 border-t border-slate-50 text-[10px] text-slate-400 flex justify-between items-center">
                                <span>Modifié: {tmpl.lastModified}</span>
                                <span className="uppercase tracking-wider font-bold bg-slate-50 px-1.5 py-0.5 rounded">{tmpl.type}</span>
                            </div>
                        </div>
                    ))}
                    <button className="border-2 border-dashed border-slate-200 rounded-lg p-6 flex flex-col items-center justify-center text-slate-400 hover:border-indigo-300 hover:text-indigo-600 transition-all">
                ...
                <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
                    <table className="min-w-full divide-y divide-slate-100">
                        <thead className="bg-slate-50">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Titre</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Slug URL</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Dernière MàJ</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {pages.map(page => (
                                <tr key={page.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4 text-xs font-mono text-slate-500">{page.slug}</td>
                                    <td className="px-6 py-4 text-xs text-slate-500">{page.lastUpdated}</td>
                                    <td className="px-6 py-4 text-right">
                                        <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${
                                            page.status === 'Published' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-yellow-50 text-yellow-700 border border-yellow-100'
                                        }`}>
                                            {page.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <button className="text-slate-400 hover:text-indigo-600 mx-1"><IconEdit className="w-4 h-4" /></button>
                                        <button className="text-slate-400 hover:text-slate-600 mx-1"><IconEye className="w-4 h-4" /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {translations.map(mod => (
                            <div key={mod.id} className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
                                ...
                                <div className="flex justify-between items-center pt-3 border-t border-slate-50 text-xs">
                                    <span className="text-slate-400">{mod.keys} clés</span>
            ...
            <SlideOver
                ...
                subtitle={editingTemplate?.type === 'Email' ? 'Format HTML supporté' : 'Texte brut uniquement'}
            >
                {editingTemplate && (
                    <div className="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <textarea 
                                rows={15}
                                className="w-full border border-slate-300 rounded-md p-3 text-sm font-mono focus:ring-2 focus:ring-indigo-500 outline-none bg-slate-50"
                                value={templateBody}
                                onChange={(e) => setTemplateBody(e.target.value)}
                            />
                            <p className="mt-2 text-xs text-slate-500">
                                Variables disponibles: <code className="bg-slate-100 px-1 rounded">{`{userName}`}</code>, <code className="bg-slate-100 px-1 rounded">{`{clinicName}`}</code>, <code className="bg-slate-100 px-1 rounded">{`{actionUrl}`}</code>.
                            </p>
                        </div>
                        <div className="pt-4 flex gap-3 border-t border-slate-100">
                            <button onClick={() => setEditingTemplate(null)} className="flex-1 py-2 text-sm font-medium text-slate-700 border border-slate-300 rounded-md hover:bg-slate-50">Annuler</button>
                            <button onClick={handleSaveTemplate} className="flex-1 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 shadow-sm flex justify-center items-center gap-2">
    ...
    const handleDrop = (e: React.DragEvent, status: string) => {
        ...
        if (draggedBroadcast && draggedBroadcast.status !== status) {
            setBroadcasts(broadcasts.map(b => 
                b.id === draggedBroadcast.id ? { ...b, status: status as any, sentAt: status === 'Sent' ? 'Just now' : undefined } : b
            ));
        }
        ...
        <div className="space-y-6 h-full flex flex-col">
            ...
            <div className="flex-1 overflow-x-auto pb-2">
                <div className="flex gap-6 h-full min-w-[800px]">
                    {columns.map(status => (
                        <div key={status} className="flex-1 flex flex-col min-w-[280px]" onDragOver={e => e.preventDefault()} onDrop={e => handleDrop(e, status)}>
                            ...
                            <div className="flex-1 space-y-3 p-1">
                                {broadcasts.filter(b => b.status === status).map(b => (
                                    <div key={b.id} draggable onDragStart={e => handleDragStart(e, b)} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm cursor-grab active:cursor-grabbing">
            ...
            {isComposeOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md relative z-10 p-6 space-y-4">
                        ...
                        <input type="text" placeholder="Titre" className="w-full border p-2 rounded" value={newBroadcast.title} onChange={e => setNewBroadcast({...newBroadcast, title: e.target.value})} />
                        <textarea placeholder="Message" className="w-full border p-2 rounded" rows={4} value={newBroadcast.message} onChange={e => setNewBroadcast({...newBroadcast, message: e.target.value})} />
                        <div className="flex justify-end gap-2">
                            <button onClick={() => setIsComposeOpen(false)} className="px-4 py-2">Annuler</button>
                            <button onClick={handleCreateBroadcast} className="px-4 py-2 bg-blue-600 text-white rounded">Créer</button>
                        </div>
                    </div>
                </div>
            )}
    ...
    <div className="space-y-6">
        <div className="flex justify-between items-center">
            <div>
                ...
                <p className="text-sm text-slate-500">Gestion des modèles LLM et coûts.</p>
            </div>
            <span className="text-xl font-bold text-slate-900">185.40 USD / jour</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
                {MOCK_AI_CONFIGS.map(config => (
                    <div key={config.id} className="bg-white p-4 rounded-lg border border-slate-200">
                        <div className="font-bold">{config.name}</div>
                        <div className="text-xs text-slate-500">{config.provider} • ${config.costPer1kTokens}/1k tk</div>
                    ...
                    {MOCK_CHURN_RISK.map(risk => (
                        <tr key={risk.id} className="hover:bg-slate-50">
                            ...
                            <td className="px-6 py-4">
                                <span className={`px-2 py-1 rounded text-xs font-bold ${risk.riskScore > 70 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>{risk.riskScore}%</span>
                            </td>
                            <td className="px-6 py-4 text-right">
                ...
                <div className="divide-y">
                    {MOCK_BACKUPS.map(bk => (
                        <div key={bk.id} className="py-3 flex justify-between items-center">
                            <div className="text-sm">{bk.clinicName} • {bk.createdAt}</div>
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {MOCK_REGIONS.map(region => (
                <div key={region.id} className="bg-slate-900 rounded-lg p-5 text-white">
                    <div className="text-xs font-bold text-slate-400 uppercase">{region.code}</div>
                    <div className="text-xl font-bold">{region.name}</div>
                    <div className="text-sm text-slate-400 mt-2">{region.activeTenants} Tenants</div>
                </div>
            ))}
        </div>
        <div className="bg-white p-6 rounded-lg border border-slate-200">
            ...
            {MOCK_CACHE_METRICS.map(c => (
                <div key={c.name} className="mb-4">
                    <div className="flex justify-between text-xs mb-1"><span>{c.name}</span><span>{c.hitRate}%</span></div>
                    <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden"><div className="bg-indigo-500 h-full" style={{width: `${c.hitRate}%`}}></div></div>
                </div>
            ))}
        </div>
    </div>
...
export const CompliancePage = () => (
    <div className="space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
            <table className="min-w-full divide-y divide-slate-100">
                <thead className="bg-slate-50">
                    <tr><th className="px-6 py-3 text-left">Tenant</th><th className="px-6 py-3 text-left">Data Location</th><th className="px-6 py-3 text-right">Status</th></tr>
                </thead>
                <tbody className="divide-y">
                    {MOCK_COMPLIANCE.map(c => (
                        <tr key={c.id}><td className="px-6 py-4">{c.clinicName}</td><td className="px-6 py-4">{c.dataLocation}</td><td className="px-6 py-4 text-right"><span className="bg-green-100 text-green-700 px-2 py-1 rounded text-xs">Compliant</span></td></tr>
                    ))}
                </tbody>
            </table>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {['Planned', 'In Progress', 'Live'].map(status => (
                <div key={status} className="space-y-4">
                    ...
                    {MOCK_FEATURE_REQUESTS.filter(f => status === 'Live' ? f.status === 'Live' : true).slice(0,2).map(f => (
                        <div key={f.id} className="bg-white p-4 rounded border shadow-sm">
                            <div className="font-bold text-sm">{f.title}</div>
                            <div className="text-xs text-slate-500 mt-2">Votes: {f.votes}</div>
                        </div>
                    ))}
                </div>
            ))}
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white border rounded-lg overflow-hidden">
            <table className="min-w-full divide-y">
                <thead className="bg-slate-50">
                    <tr><th className="px-6 py-3 text-left">Event</th><th className="px-6 py-3 text-left">IP</th><th className="px-6 py-3 text-right">Threat</th></tr>
                </thead>
                <tbody className="divide-y">
                    {MOCK_SECURITY_EVENTS.map(s => (
                        <tr key={s.id}><td className="px-6 py-4">{s.reason}</td><td className="px-6 py-4 font-mono text-xs">{s.ip}</td><td className="px-6 py-4 text-right">{s.threatLevel}</td></tr>
                    ))}
                </tbody>
            </table>
        </div>
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-6 border rounded-lg">
                ...
                {MOCK_JOBS.map(j => (
                    <div key={j.id} className="flex justify-between py-2 border-b"><span>{j.name}</span><span className="text-xs bg-blue-50 text-blue-600 px-2 rounded">Running</span></div>
                ))}
            </div>
        </div>
    </div>
      ...
      <div className="space-y-6">
         <div className="flex gap-4 border-b">
            {['webhooks', 'api', 'flags', 'sql'].map(t => (
                <button key={t} onClick={() => setSubTab(t as any)} className={`pb-2 px-4 text-sm font-medium ${subTab === t ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-slate-500'}`}>{t.toUpperCase()}</button>
            ))}
         </div>
         {subTab === 'webhooks' && (
         ...
         {subTab === 'api' && (
             <div className="grid grid-cols-3 gap-4">
                 {MOCK_API_METRICS.map(m => (
                     <div key={m.endpoint} className="p-4 border rounded bg-slate-50">
                         <div className="text-xs text-slate-500 truncate">{m.endpoint}</div>
...
export const SaaSAdministration = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'users': return <SaaSUsersPage />;
          ...
          {navItems.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md transition-colors ${
                activeTab === item.id ? 'bg-white text-indigo-700 shadow-sm border border-slate-200' : 'text-slate-600 hover:bg-slate-100'
              }`}
            >
