    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <div className="bg-slate-100 p-1 rounded-[20px] flex text-sm font-medium">
            <button
              onClick={() => setActiveTab('list')}
              className={`px-3 py-1.5 rounded-sm transition-colors ${activeTab === 'list' ? 'bg-white text-slate-900 ' : 'text-slate-500 hover:text-slate-900'}`}
            >
              Liste
            </button>
            <button
              onClick={() => setActiveTab('chart')}
              className={`px-3 py-1.5 rounded-sm transition-colors ${activeTab === 'chart' ? 'bg-white text-slate-900 ' : 'text-slate-500 hover:text-slate-900'}`}
            >
              Odontogramme
            </button>
          </div>
      ...
      {/* Main Content */}
      {activeTab === 'chart' ? (
        <div className="bg-white rounded-[20px] border border-slate-200  p-6 sm:p-8">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-semibold text-slate-800">Visualisation Clinique</h3>
            <select
              className="border-slate-300 rounded-[20px] text-sm p-2"
        ...
        <div className="space-y-4">
          {/* Filters */}
          <div className="bg-white p-2 rounded-[20px] border border-slate-200  flex items-center justify-between">
            <div className="relative flex-1 max-w-sm">
              ...
              <input
                ...
                className="block w-full pl-9 pr-3 py-1.5 border-none rounded-[20px] leading-5 bg-transparent placeholder-gray-400 focus:outline-none focus:ring-0 text-sm"
              />
            </div>
          </div>
      ...
      >
        <form onSubmit={handleCreatePlan} className="flex flex-col h-full bg-slate-50">
          <div className="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            <div className="space-y-4 bg-white p-4 rounded-[20px]  border border-slate-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    required
                    className="w-full border-slate-300 rounded-[20px] text-sm p-2"
                    value={selectedPatientId}
                    onChange={(e) => setSelectedPatientId(e.target.value)}
                  >
            ...
            <div className="bg-white p-4 rounded-[20px]  border border-slate-200">
              ...
              <div className="border border-slate-100 rounded-[20px] p-2 bg-slate-50">
                <Odontogram statusMap={odontogramState} onToothUpdate={handleToothUpdate} />
            ...
            <div className="space-y-4">
              ...
              {phases.map((phase, idx) => (
                <div
                  key={phase.id}
                  className="bg-white border border-slate-200 rounded-[20px] p-4 "
                >
          ...
          <div className="p-4 bg-white border-t border-slate-200 flex justify-between items-center">
            ...
            <div className="flex gap-3">
              ...
              <button
                type="submit"
                className="px-4 py-2 bg-blue-600 text-white rounded-[20px] text-sm font-medium hover:bg-blue-700 "
              >
                Créer le plan
              </button>
            </div>
        ...
        {selectedPlan && (
          <div className="flex flex-col h-full bg-slate-50">
                    ...
                    {selectedPlan.sessions
                      ...
                      .map((session, idx) => (
                        <div key={session.id} className="relative pl-10">
                          <div
                            className={`absolute left-2.5 top-1.5 w-3.5 h-3.5 rounded-full border-2 border-white  z-10 ${
                              session.status === 'Completed' ? 'bg-green-500' : 'bg-blue-400'
                            }`}
                          ></div>
                          <div className="bg-white p-4 rounded-[20px]  border border-slate-200 hover:border-blue-200 transition-colors">
            ...
            <div className="p-4 bg-white border-t border-slate-200 flex gap-3">
              <button className="flex-1 bg-slate-100 text-slate-600 hover:bg-slate-200 py-2.5 rounded-[20px] text-sm font-bold transition-colors">
                Imprimer Devis
              </button>
              <button
                onClick={() => setSelectedPlan(null)}
                className="flex-1 bg-slate-900 text-white hover:bg-slate-800 py-2.5 rounded-[20px] text-sm font-bold  transition-all"
              >
                Fermer
              </button>
            </div>
