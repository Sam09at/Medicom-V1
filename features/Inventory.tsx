...
import { IconSearch, IconPlus, IconArchive, IconAlertTriangle, IconCheckCircle, IconX, IconCheck } from '../components/Icons';
...
export const Inventory = () => {
  ...
  const [orders, setOrders] = useState<Order[]>([
      { id: 'CMD-001', items: [{ name: 'Gants Latex (M)', quantity: 20 }], supplier: 'MedicalExpress', date: '2024-01-24', status: 'Pending' }
  ]);
  ...
  const handleCreateItem = (e: React.FormEvent) => {
      ...
      const item: InventoryItem = {
          ...
          lastRestock: new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })
      };
      setItems([item, ...items]);
      setIsCreateOpen(false);
      setNewItem({ name: '', category: 'Consumable', quantity: '', minThreshold: '', unit: 'Boîtes', supplier: '' });
  };
    ...
    <div className="space-y-6 font-sans relative">
      ...
      {isOrderModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setIsOrderModalOpen(false)}></div>
              <div className="bg-white rounded-lg shadow-xl w-full max-w-sm relative z-10 p-6 space-y-4">
                  <div className="flex justify-between items-center">
                      <h3 className="text-lg font-bold text-slate-900">Commander Article</h3>
                      <button onClick={() => setIsOrderModalOpen(false)}><IconX className="w-5 h-5 text-slate-400" /></button>
                  </div>
                  <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1">Article</label>
                      <input type="text" value={selectedItemForOrder} disabled className="w-full bg-slate-50 border border-slate-200 rounded p-2 text-sm text-slate-600" />
                  </div>
                  <div>
                  ...
                  <button onClick={submitOrder} className="w-full bg-blue-600 text-white py-2 rounded-md font-medium hover:bg-blue-700">Confirmer la commande</button>
              </div>
          </div>
      )}
          ...
          <form onSubmit={handleCreateItem} className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div>
                      ...
                      <select 
                        className="w-full border border-slate-300 rounded-md p-2 text-sm bg-white outline-none"
              ...
              <div className="pt-4 flex gap-3">
                  <button type="button" onClick={() => setIsCreateOpen(false)} className="flex-1 py-2 border border-slate-300 rounded-md text-slate-700 font-medium hover:bg-slate-50">Annuler</button>
                  <button type="submit" className="flex-1 py-2 bg-blue-600 text-white rounded-md font-medium hover:bg-blue-700 shadow-sm">Créer Référence</button>
              </div>
          </form>
      </SlideOver>
        ...
        <div className="flex gap-3">
            <div className="flex bg-slate-100 p-1 rounded-md">
                <button 
                    ...
                    className={`px-3 py-1.5 text-xs font-medium rounded-sm transition-all ${activeTab === 'stock' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
                >
                    Inventaire
                </button>
                <button 
                    onClick={() => setActiveTab('orders')}
                    className={`px-3 py-1.5 text-xs font-medium rounded-sm transition-all ${activeTab === 'orders' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
                >
                    Commandes
                </button>
            </div>
      ...
      {activeTab === 'stock' ? (
        <>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <div className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm flex flex-col">
                    ...
                    <div className="space-y-3 flex-1 overflow-auto">
                        {items.filter(i => i.quantity <= i.minThreshold).length === 0 ? (
                            ...
                            items.filter(i => i.quantity <= i.minThreshold).map(item => (
                                <div key={item.id} className="flex justify-between items-center p-2 bg-red-50 rounded border border-red-100">
                                    ...
                                    <button onClick={() => openOrderModal(item.name)} className="text-xs bg-white text-red-600 border border-red-200 px-2 py-1 rounded hover:bg-red-50">Commander</button>
                                </div>
                            ))
                        )}
                    </div>
                ...
                <div className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm flex flex-col justify-center items-center text-center">
                ...
                <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50/50">
                    <div className="flex gap-2">
                        {['All', 'Consumable', 'Instrument'].map(cat => (
                            <button 
                                ...
                                className={`px-3 py-1.5 text-xs font-medium rounded-full border transition-colors ${filter === cat ? 'bg-white text-slate-900 border-slate-300 shadow-sm' : 'text-slate-500 border-transparent hover:bg-slate-100'}`}
                            >
                    ...
                    <div className="relative w-64">
                        ...
                        <input 
                            ...
                            className="w-full pl-9 pr-3 py-1.5 text-sm border border-slate-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500" 
                        />
                    </div>
                </div>
                <table className="min-w-full divide-y divide-slate-100">
                    <thead>
                        <tr className="bg-slate-50">
                            ...
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Fournisseur</th>
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Dernier Réassort</th>
                    ...
                    <tbody className="bg-white divide-y divide-slate-100">
                        {filteredItems.map(item => (
                            <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                <td className="px-6 py-4 whitespace-nowrap font-medium text-sm text-slate-900">{item.name}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-xs text-slate-500">{item.category === 'Consumable' ? 'Consommable' : 'Instrument'}</td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className={`text-xs font-bold px-2 py-0.5 rounded border ${getStatusColor(item.quantity, item.minThreshold)}`}>
                                            {item.quantity} {item.unit}
                                        </span>
                                    </div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">{item.supplier}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{item.lastRestock}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <button onClick={() => openOrderModal(item.name)} className="text-blue-600 hover:text-blue-900 text-xs font-medium">Commander</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
        ...
        <div className="bg-white rounded-md border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <tr key={order.id} className="hover:bg-slate-50 transition-colors">
                            ...
                            <td className="px-6 py-4">
                                <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold border ${
                                    ...
                                    {order.status === 'Pending' ? 'En attente' : order.status === 'Received' ? 'Reçue' : 'Annulée'}
                                </span>
                            </td>
                            <td className="px-6 py-4 text-right">
                                {order.status === 'Pending' && (
                                    <button 
                                        onClick={() => setOrders(orders.map(o => o.id === order.id ? { ...o, status: 'Received' } : o))}
            ...
            {orders.length === 0 && <div className="p-8 text-center text-slate-400 text-sm">Aucune commande en cours.</div>}
        </div>
      )}
    </div>
  );
